...
if __name__ == '__main__':
    ...
    for softmaxBeta in softmaxBetaList:
        ...
        statDF = pd.DataFrame()
        statDF['likelihood'] = df.apply(lambda x: calTrajLikelihood(eval(x['trajectory']), eval(x['aimAction']), eval(x['target1']), eval(x['target2'])), axis=1)
